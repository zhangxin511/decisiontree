<cfcomponent>
	...
	<cffunction name="login" access="public" returntype="void" output="true">
		<cfargument name="rc" type="struct" required="yes" />
		...
		<cfif trim(rc.email) eq "">
			 <cfset rc.message = ['<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter usename!</div>'] />
			 <cfset variables.fw.redirect('login','message') />
		</cfif>
		
		<cfif trim(rc.password) eq "">
			 <cfset rc.message = ['<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter passowrd!</div>'] />
			 <cfset variables.fw.redirect('login','message') />
		</cfif>		
		...
		<cfif not userValid>
			<cfset rc.message = ['<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button>Invalid username or password!</div>'] />
			<cfset variables.fw.redirect('login','message') />
		</cfif>
